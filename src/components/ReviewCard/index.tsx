...
const ReviewCard: React.FC<Props> = ({ id, name, image, description }) => {
  ...
  return (
    <S.Card>
      <S.User>
        <S.Image>
          <source srcSet={`@images/reviews/${image}?webp`} type="image/webp" />
          <source srcSet={`@images/reviews/${image}`} type="image/jpg" />
          <img src={`@images/reviews/${image}`} loading="lazy" alt={name} />
        </S.Image>
        <S.Name>{name}</S.Name>
      </S.User>
      <S.Text>
